...
# num_obs_sac = sac_results[:, 2]
obs_fraction_sac = sac_results[:, 3]

obs_fraction_by_num_targets_sac = [[], [], [], [], []]
for i in range(0, np.shape(obs_fraction_sac)[0]):
    # Append this obs_fraction to the list of obs_fractions fo this number of targets
    obs_fraction_by_num_targets_sac[int(num_targets_sac[i])-1].append(obs_fraction_sac[i])

# Series 2
hc_results = np.loadtxt('obs_distribution_hc.csv', delimiter=',', skiprows=1)

num_targets_hc = hc_results[:, 0]
# num_obs_hc = hc_results[:, 2]
obs_fraction_hc = hc_results[:, 3]

obs_fraction_by_num_targets_hc = [[], [], [], [], []]
for i in range(0, np.shape(obs_fraction_hc)[0]):
    # Append this obs_fraction to the list of obs_fractions fo this number of targets
    obs_fraction_by_num_targets_hc[int(num_targets_hc[i])-1].append(obs_fraction_hc[i])
...
for i in range(5):
    y = obs_fraction_by_num_targets_sac[i]
...
for i in range(5):
    y = obs_fraction_by_num_targets_hc[i]
    x = np.random.normal(1+i, 0.04, size=len(y))
...
fig.suptitle(
    f"Fraction of Budget Spent Observing Each Target", color="#333333"
)
...
for axis in ax:
    ...
    axis.set_ylabel(
        "Fraction of Budget Spent Observing Each Individual Target", color="#333333"
    )
