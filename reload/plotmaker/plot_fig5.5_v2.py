"""
Presents the number of observastions on each target
"""
...
num_obs_by_num_targets_sac = [[], [], [], [], []]
for i in range(0, np.shape(num_obs_sac)[0]):
    # Append this obs_fraction to the list of obs_fractions fo this number of targets
    num_obs_by_num_targets_sac[int(num_targets_sac[i])-1].append(num_obs_sac[i])
...
num_obs_by_num_targets_hc = [[], [], [], [], []]
for i in range(0, np.shape(num_obs_hc)[0]):
    # Append this obs_fraction to the list of obs_fractions fo this number of targets
    num_obs_by_num_targets_hc[int(num_targets_hc[i])-1].append(num_obs_hc[i])
...
for i in range(5):
    y = num_obs_by_num_targets_sac[i]
...
fig.suptitle(
    f"Number of Observations on Each Target", color="#333333"
)
...
for axis in ax:
    ...
    axis.set_ylabel(
        "Number of Observations Each Individual Target", color="#333333"
    )
